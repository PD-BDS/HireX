...
def _resolve_path(path_value: str) -> Path:
    ...
    # Intercept paths pointing to the default knowledge store location
    # and redirect them to the configured DATA_ROOT.
    if "knowledge_store" in path_value:
        # Extract the part after 'knowledge_store/'
        parts = path_value.replace("\\", "/").split("knowledge_store/")
        if len(parts) > 1:
            relative_part = parts[1]
            return (DATA_ROOT / relative_part).resolve()
